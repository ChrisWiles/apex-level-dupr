...
- **How hard "not found" was actually tried**: the original ~85 unmatched players (from the current 2-week window) went through five separate passes: (1) nickname/hyphen/word-order variants, (2) a worldwide (not just Texas) search radius, (3) rare-surname isolation with up to ~10 variants per name, (4) mining every account that changed its display name across the ~70 scrapes (this is what resolved "SR T" → "Raj T" and "Lacie henry" → "Lacie Petsch" -- verified by checking the account showed a single clean rename cutover on one scrape date, not two names alternating, before trusting it), and (5) a final top-up pass ensuring every remaining name reached a genuine 9-10 distinct search attempts (transliteration variants, swapped name order, vowel-drops, surname/first-name-alone). A DUPR club-roster cross-reference and a plain web search were also tried and came up empty (Apex Pickleball Clubs Cedar Park has no registered DUPR club page). After all of that, the remaining unmatched break down into: players with only initials on file (no technique can recover an identity from two letters), players confirmed absent from DUPR (zero hits worldwide even on their most distinctive name fragment), a few "family cluster" cases where a relative with the same rare surname plays locally but the specific person doesn't (not treated as a match), and common-name collisions with no standout candidate.
